...
ANIMAL_CLASSES = [
    'bo', 'buom', 'cho', 'cuu', 'ga', 'meo', 'ngua', 'nhen', 'soc', 'sutu', 'voi'
]
...
def animal_search():
    ...
    if request.method == 'POST':
        ...
        if selected_animal in ANIMAL_CLASSES:
            animal_dir = os.path.join('static', 'raw', selected_animal)
            if os.path.exists(animal_dir):
                all_images = [f for f in os.listdir(animal_dir) if f.lower().endswith(('jpg', 'jpeg', 'png'))]
                if len(all_images) > 0:
                    images = sample(all_images, min(5, len(all_images)))
                    images = [f'/static/raw/{selected_animal}/{img}' for img in images]
    return render_template('animal_search.html', animals=ANIMAL_CLASSES, images=images, selected_animal=selected_animal)
